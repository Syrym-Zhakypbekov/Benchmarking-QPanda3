...
Table~\ref{tab:results} summarizes anomaly detection results on IoT sensor data. Quantum VQC achieves 92.3\% ± 1.8\% accuracy with only 18 parameters (6 qubits, 3 layers), compared to classical models requiring 100-2000+ parameters. Statistical significance testing confirms quantum advantage (p $<$ 0.001). Figure~\ref{fig:model_comparison} provides a comprehensive comparison of quantum vs classical models across multiple metrics, demonstrating the parameter efficiency advantage of quantum approaches.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{../results/figures/model_comparison.png}
\caption{Comprehensive model comparison: Quantum VQC vs Classical ML models. Left: Accuracy comparison. Right: Parameter efficiency analysis.}
...
Figure~\ref{fig:iot_timeseries} shows time-series analysis of vibration patterns, demonstrating how anomalies manifest as spikes in sensor readings. The time-series visualization reveals temporal patterns in sensor data, with anomalies clearly visible as deviations from normal vibration baselines.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{../results/figures/iot_time_series_analysis.png}
\caption{Time-series analysis of IoT sensor data: X, Y vibration components and temperature readings with anomaly detection. Red markers indicate detected anomalies.}
\label{fig:iot_timeseries}
\end{figure}

\subsection{Feature Correlation Analysis}

Figure~\ref{fig:iot_correlation} presents the feature correlation matrix, revealing strong correlations between vibration components that inform our quantum encoding strategy. Understanding these correlations enables effective dimensionality reduction via PCA while preserving discriminative information.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{../results/figures/iot_correlation_heatmap.png}
\caption{Feature correlation matrix for IoT sensor data, showing relationships between vibration components (X, Y, Z) and environmental sensors (Temperature, Humidity, Pressure).}
\label{fig:iot_correlation}
\end{figure}

\subsection{Feature Distribution Analysis}

Figure~\ref{fig:iot_features} shows the distribution of key features for normal vs anomalous sensor readings. The clear separation between distributions validates the discriminative power of vibration magnitude and environmental features for anomaly detection.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{../results/figures/iot_feature_distributions.png}
\caption{Feature distribution analysis: Comparison of normal vs anomalous sensor readings across X, Y, Z vibration components and environmental sensors.}
\label{fig:iot_features}
\end{figure}

\subsection{Aftershock Event Analysis}

Figure~\ref{fig:aftershock} analyzes aftershock events in the IoT sensor data, demonstrating their correlation with anomalous vibration patterns. This analysis informs our anomaly detection strategy by identifying seismic events as a key indicator of structural anomalies.

\begin{figure}[!t]
\centering
\includegraphics[width=0.48\textwidth]{../results/figures/iot_aftershock_analysis.png}
\caption{Aftershock event analysis: Timeline, magnitude comparison, and correlation with anomaly detection.}
...
All source code, experimental scripts, and data necessary to reproduce the results presented in this paper are publicly available at:

\texttt{https://github.com/Syrym-Zhakypbekov/Benchmarking-QPanda3}

The repository includes:
\begin{itemize}
    \item Complete source code for all experiments (Python 3.12)
    \item Data loading and preprocessing scripts
    \item Model implementations (quantum VQC and classical baselines)
    \item Jupyter notebooks documenting the complete research workflow
    \item All experimental results (figures and CSV data files)
    \item Comprehensive documentation and reproducibility instructions
\end{itemize}

IoT sensor data used in this study is available in the repository (`1.exl.csv`) or can be generated synthetically using the provided data generation script (`src/data/iot_sensor_data.py`) for full reproducibility. All experiments use fixed random seeds (random\_state=42) to ensure reproducibility.

For detailed methodology and step-by-step reproduction instructions, see the repository documentation (`docs/METHODOLOGY.md`).
